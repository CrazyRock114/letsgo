...
export default function GoGamePage() {
      ...
      try {
        ...
        if (data.game) {
          ...
          setSpectatorGame({
            ...
            config: game.config as Record<string, unknown> | undefined,
            updated_at: game.updated_at as string | undefined,
          });
          ...
          if (game.status === 'finished') {
            const scores = calculateFinalScore(game.final_board as Board || createEmptyBoard(bs));
            setGameResult(scores);
          }
          // 同步后台配置到 spectator 页面显示
          const cfg = game.config as { opponent?: { engine?: string; difficulty?: string } } | undefined;
          if (cfg?.opponent?.engine) {
            setEngine(cfg.opponent.engine as EngineId);
          } else if (game.engine) {
            setEngine(game.engine as EngineId);
          }
          if (cfg?.opponent?.difficulty) {
            setDifficulty(cfg.opponent.difficulty);
          ...
          setLastMove(null);
          setScore({ black: 0, white: getKomi(boardSize) });
        ...
        if (!cancelled) console.warn('[spectator] load error:', err);
      }
    }

    loadSpectatorGame();
    const interval = setInterval(loadSpectatorGame, 3000);
  ...
  const handleMove = useCallback(async (row: number, col: number) => {
    // 围观模式禁用落子
    if (IS_SPECTATOR) { toast.info('围观模式，不能落子'); return; }
    // 禁用状态提示
        ...
        <div className="flex items-center justify-center gap-2 mt-2 flex-wrap">
          ...
          {BOARD_SIZES.map(({ size, label, desc }) => (
            <Button
              ...
              disabled={isReplayMode || IS_SPECTATOR}
            >
            ...
            {ENGINE_OPTIONS.map(({ id, name, desc }) => {
              ...
              return (
                <button
                  ...
                  disabled={isReplayMode || !avail || isLoading || needLogin || IS_SPECTATOR}
      ...
      <div className="max-w-7xl mx-auto px-3 pb-3 grid grid-cols-1 lg:grid-cols-12 gap-3 lg:h-[calc(100vh-120px)]">
        ...
        <div className="lg:col-span-3 space-y-3 lg:overflow-y-auto lg:pr-1">
            ...
            <CardContent className="py-2 px-3">
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium text-blue-800">围观AI玩家挑战AI</div>
                  <div className="text-xs text-blue-600">
                    {spectatorGame ? (
                      spectatorGame.status === 'running' ? (
                        <span className="flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                          后台自动对弈中
                        </span>
                      ) : spectatorGame.status === 'paused' ? (
                        <span className="flex items-center gap-1">
                  ...
                  {spectatorGame?.title && (
                    <div className="text-[10px] text-blue-500 mt-0.5 truncate">{spectatorGame.title}</div>
                  )}
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="py-3">
              <div className="flex justify-around text-center">
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-gray-500">黑方(AI玩家)</span>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-gray-500">白方(AI对手)</span>
          ...
          <Card className="bg-white/90 shadow-lg">
            <CardContent className="py-3 grid grid-cols-2 gap-1.5">
              <Button onClick={restartGame} variant="outline" size="sm" className="gap-1 h-8 text-xs" disabled={isReplayMode || IS_SPECTATOR}>
                <RotateCcw className="w-3 h-3" /> 重新开始
              </Button>
              <Button onClick={undoMove} variant="outline" size="sm" className="gap-1 h-8 text-xs" disabled={history.length === 0 || isReplayMode || isAIThinking || IS_SPECTATOR}>
                <RotateCcw className="w-3 h-3" /> 悔棋
              </Button>
              <Button onClick={passMove} variant="outline" size="sm" className="gap-1 h-8 text-xs" disabled={currentPlayer !== playerColor || isAIThinking || isReplayMode || gameEnded || IS_SPECTATOR}>
              ...
              <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" className="gap-1 h-8 text-xs" disabled={history.length === 0 || IS_SPECTATOR}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-3">
                    ...
                    <Button onClick={saveGame} disabled={isSaving || IS_SPECTATOR} className="w-full bg-amber-700 hover:bg-amber-800">
                      {isSaving ? '保存中...' : '确认保存'}
                    </Button>
                    {/* 自动保存开关 */}
                    {user && (
                      <div className="flex items-center justify-between pt-1 border-t">
                        ...
                        <button
                          ...
                          disabled={IS_SPECTATOR}
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${autoSave ? 'bg-amber-600' : 'bg-gray-300'} ${IS_SPECTATOR ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
              ...
              <Dialog open={showLoadDialog} onOpenChange={(open) => { setShowLoadDialog(open); if (open) loadGames(); }}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" className="gap-1 h-8 text-xs" disabled={IS_SPECTATOR}>
                      ...
                      <div className="space-y-2">
                        {savedGames.map(g => (
                          <div key={g.id} className="flex items-center justify-between p-2 border rounded-lg hover:bg-gray-50">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => deleteGame(g.id!)} disabled={IS_SPECTATOR} className="text-red-400 hover:text-red-600">
            ...
            <Card className="bg-blue-50 border-blue-200">
              ...
              <CardContent>
                ...
                <div className="flex items-center justify-center gap-2">
                  <Button size="sm" variant="outline" onClick={() => replayStep(-1)} disabled={replayIndex <= 0 || IS_SPECTATOR}>
                    <ChevronLeft className="w-4 h-4" />
                  </Button>
                  <span className="text-sm font-medium min-w-[60px] text-center">{replayIndex}/{replayMoves.length}</span>
                  <Button size="sm" variant="outline" onClick={() => replayStep(1)} disabled={replayIndex >= replayMoves.length || IS_SPECTATOR}>
                    <ChevronRight className="w-4 h-4" />
                  </Button>
                </div>
                <Button size="sm" variant="destructive" className="w-full mt-2" onClick={exitReplay} disabled={IS_SPECTATOR}>
                  退出复盘
                </Button>
                {replayIndex > 0 && (
                  <Button size="sm" className="w-full mt-2 bg-green-600 hover:bg-green-700 text-white" onClick={resumeFromReplay} disabled={IS_SPECTATOR}>
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-yellow-50 border-amber-200">
            <CardContent className="py-3">
              <Button
                ...
                disabled={isReplayMode || isTeachStreaming || gameEnded || isAIThinking || teachUsedCount >= MAX_TEACH_PER_GAME || IS_SPECTATOR}
              >
        ...
        <div className="lg:col-span-5 space-y-3 lg:overflow-y-auto">
          <div className="flex justify-center" style={{ touchAction: 'manipulation' }}>
            <GoBoard board={board} boardSize={boardSize} lastMove={lastMove} showHint={showHint} isAIThinking={isAIThinking} isReplayMode={isReplayMode} isSpectator={IS_SPECTATOR} onMove={handleMove} />
          </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="text-center space-y-3 py-2">
            ...
            <div className="flex gap-3 justify-center">
              <Button onClick={saveGame} disabled={isSaving || IS_SPECTATOR} variant="outline" size="lg" className="px-6">
                {isSaving ? '保存中...' : '保存棋局'}
              </Button>
              <Button onClick={restartGame} disabled={IS_SPECTATOR} size="lg" className="bg-amber-700 hover:bg-amber-800 text-white px-6">
                再来一局
              </Button>
            </div>
          </div>
