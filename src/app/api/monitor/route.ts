    ...
    return NextResponse.json({
      ...
      games: {
        ...
        activeSessions: activeSessions.map(s => ({
          player: s.nickname || `User#${s.userId}`,
          engine: s.engine,
          boardSize: s.boardSize,
          difficulty: s.difficulty,
          totalMoves: s.totalMoves,
          lastMoveAt: new Date(s.lastMoveAt).toISOString(),
          startedAt: new Date(s.startedAt).toISOString(),
        })),
      },
      engineQueue: {
        total: engineData.queueLength,
        isProcessing: engineData.isProcessing,
        katago: {
          queueLength: engineData.kataGoQueueLength,
          processing: engineData.kataGoProcessing,
        },
        gnugo: {
          queueLength: engineData.gnuGoQueueLength,
          processing: engineData.gnuGoProcessing,
        },
      },
      usage: {
        pointsUsedLastHour,
